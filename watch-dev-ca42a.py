...
def wait_for_device_ready():
    ...
    try: # <--- THIS IS THE START OF THE TRY BLOCK FOR THE MAIN LOOP
        while time.time() - start_total_wait_time < MAX_TOTAL_WAIT_TIME_SECONDS:
            ...
            if red_led_value is None or green_led_value is None:
                print(f"Failed to get one or both LED values. Retrying in {POLLING_INTERVAL_SECONDS}s...", file=sys.stderr)
                time.sleep(POLLING_INTERVAL_SECONDS)
                continue

            print(f"Current LED values: Red='{red_led_value}', Green='{green_led_value}' (elapsed: {int(time.time() - start_total_wait_time)}s)")

            if red_led_value == EXPECTED_R_VALUE and green_led_value == EXPECTED_G_VALUE:
                print(f"Device LEDs are Red={EXPECTED_R_VALUE}, Green={EXPECTED_G_VALUE}. Device is ready!")
                return True

            time.sleep(POLLING_INTERVAL_SECONDS)

        print(f"Error: Device did not become ready within {MAX_TOTAL_WAIT_TIME_SECONDS} seconds.", file=sys.stderr)
        print(f"Last LED values: Red='{red_led_value}', Green='{green_led_value}' (if available)", file=sys.stderr)
        return False
