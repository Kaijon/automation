...
def run_command(cmd, dry_run=False, verbose=False):
    ...
    for idx in range(3):
        ...
        print_stderr(f"[error] no. {idx} running command: {' '.join(cmd)}")
        print_stderr(stderr.decode("utf-8"))
        sleep(5)
    sys.exit(1)


class BitrateStats: # pylint: disable=too-many-instance-attributes
    def __init__( # pylint: disable=too-many-arguments
        ...
        # missing attributes
        self.contains_audio = 0
        self.avg_bitrate = 0
        self.avg_bitrate_over_chunks = []
    ...
    def _calculate_frame_sizes(self): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        """
        ...
        cmd = [
            ...
            "-show_packets",
            "-show_entries",
            "packet=pts_time,dts_time,duration_time,size,flags,stream_index : stream=index,codec_type,codec_name,width,height",
            "-of",
            "json",
            self.input_file,
        ]
        ...
        if video_stream:
            ...
            self.codec_name = video_stream.get("codec_name", 0)
        ...
        for packet_info in packets:
            ...
            try:
                ...
                print_stderr("Malformed packet_info['pts_time'], defaulting to NaN")
                pts = "NaN"
    ...
    def _fix_durations(self, ret):
        ...
        for i in range(len(ret) - 1):
            ...
            if isinstance(curr_pts, str) or isinstance(next_pts, str):
                print(f"FUCK curr_pts: {curr_pts}; next_pts: {next_pts}")
    ...
    def _collect_chunks(self):
        ...
        This is cached.
        """
        if self._chunks and len(self._chunks) > 0:
            return self._chunks
...
def parse_input_data():
    parser = argparse.ArgumentParser(
        ...
        description="stream_bitrate_stats v",
    )
